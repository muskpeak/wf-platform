...
export function parseWeb3Error(error: any, abis: any[] = []): ParsedWeb3Error {
  ...
  if (
    ...
    return { kind: 'user_rejected', message: 'You canceled the request in your wallet.', raw: rawMessage };
  }

  // 2. Detect Generic Gas / Funds issues
  if (lowerRaw.includes('insufficientbalance') || lowerRaw.includes('insufficient balance')) {
    return { kind: 'insufficient_funds', message: 'Insufficient balance to complete this transaction.', raw: rawMessage };
  }
  if (lowerRaw.includes('insufficient funds') || lowerRaw.includes('insufficientgas') || lowerRaw.includes('intrinsic gas too low')) {
    return { kind: 'insufficient_gas', message: 'Insufficient gas to complete this transaction.', raw: rawMessage };
  }
    ...
    if (revertError instanceof ContractFunctionRevertedError) {
      ...
      if (errorName && errorName !== 'Unknown' && errorName !== 'Error') {
        return {
          ...
          message: errorName, // e.g., 'ERC20InsufficientBalance'
          raw: rawMessage
        };
      }
    }
  ...
  return {
    kind: 'unknown',
    message: 'Something went wrong on the blockchain. Please try again.',
    raw: rawMessage
  };
}
