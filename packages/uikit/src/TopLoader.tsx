...
function TopLoaderContent() {
  ...
  // 允许外部通过 window 事件取消进度条（例如未登录拦截时）
  useEffect(() => {
    const handleCancel = () => cancelLoading()
    window.addEventListener('toploader:cancel', handleCancel)
    return () => window.removeEventListener('toploader:cancel', handleCancel)
  }, [])
